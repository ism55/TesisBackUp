...
\begin{figure}[H]
	...
	\caption[Esquema de indentificación de los motores]{Esquema de indentificación de los motores del brazo y la pinza neumática.}
...
Con respecto a las acciones del sistema, este cuenta con las siguientes: Movimiento de los seis motores en sentido horario y antihorario a través de un controlador PID y señales PWM, accionamiento de una pinza neumática de dos estados (abierto/cerrado), ajuste de los parámetros del controlador PID y de las constantes de calibración para el movimiento de los motores y habilitación/deshabilitación de los puentes H.
...
\begin{figure}[H]
	...
	\hfill	
		\begin{minipage}[c]{0.3\textwidth}
			...
			(c) -135°
	\end{minipage}
	\caption{Límites máximos de movimiento para el motor A (q1).}
	\label{fig:maximoA}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.3\textwidth}
		...
		(c) -135°
	\end{minipage}
	\caption{Límites máximos de movimiento para el motor B (q2).}
	\label{fig:maximoB}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.3\textwidth}
		...
		(c) -135°
	\end{minipage}
	\caption{Límites máximos de movimiento para el motor C (q3).}
	\label{fig:maximoC}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.3\textwidth}
		...
		(c) -66°
	\end{minipage}
	\caption{Límites máximos de movimiento para el motor D (q4).}
	\label{fig:maximoD}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.3\textwidth}
		...
		(c) -66°
	\end{minipage}
	\caption{Límites máximos de movimiento para el motor E (q5).}
	\label{fig:maximoE}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.3\textwidth}
		...
		(c) -66°
	\end{minipage}
	\caption{Límites máximos de movimiento para el motor F (q6).}
	\label{fig:maximoF}
\end{figure}
...
El sistema realiza una acción de control PID en tres articulaciones principales (hombro, codo y muñeca) con ayuda de los puentes H de la figura \ref{fig:actuadores} para permitir el movimiento de los motores en sentido horario o antihorario. Para esto se configuran los parámetros desde un software (Robocom) y se envían al controlador en una trama de tres bytes vía puerto serial, la cual será descrita en secciones posteriores.\\
...
En el panel inferior, según se observa en la figura \ref{fig:robocomopen}, se encuentran las configuraciones generales de comunicación, el accionamiento de la pinza neumática, la habilitación de los puentes H y un menú que corresponde a la configuración del graficador del error.

En la pestaña 'Comunicación' (figura \ref{fig:robocomopen} sección 3), se selecciona el puerto COM al que se conecta originalmente el controlador del brazo.

La pestaña 'Gripper' (figura \ref{fig:robocomopen} sección 4) posee un botón que activa o desactiva la pinza neumática.

La pestaña 'Puentes H' (figura \ref{fig:robocomopen} sección 5) permite habilitar o deshabilitar los puentes H de los motores principales.
...
Donde $K_{pro}$ es una constante proporcional de calibración que es utilizada para convertir el ángulo de binario a grados, medida en grados/bit y su valor programado por defecto es 0.33 grados/bit para los tres motores principales (A, B, C). Para el caso de los motores de la mano (D, E, F), el valor es fijo y vale 0.035 grados/bit.

$ K_{offset}$ es el error de corrección de la conversión a bits y es medida en bits. Su valor inicial es 0 para los motores A y C, y 39 para el motor B. Para los motores de la mano (D, E, F), el valor es fijo y vale 5682 bit.
...
Al acceder a la pestaña 'Menú' que aparece en la figura \ref{fig:robocomopen} se puede acceder a la opción 'Calibración', y en la figura \ref{fig:robocomcali} puede apreciarse mejor la información sobre $ K_{offset}$ y $K_{pro}$ descritas anteriormente.

\begin{figure}[H]
	\centering
	\includegraphics[scale=1]{img/robocom_cali}
	\caption[Configuración de constantes de calibración]{Configuración de constantes de calibración del software Robocom}
...
\begin{itemize}
	\item La trama de comunicación será de 24 bits distribuidos: 8 bits para el comando y 16 bit para el dato a transmitir, como se indica en la tabla \ref{TAB:estructura}.
	
	\item La cabecera o 'comando' de la trama puede tomar cualquier valor referido en la tabla \ref{TAB:comandocliente} en la columna 'Comando'. El valor será de 8 bit e indicará qué instrucción ejecutará el controlador.
	
	\item El dato será el valor equivalente en hexadecimal del ángulo que se desea mover y será guardado en una variable de 16 bit 'Vdigital'.
	...
	\item Para todos los motores, se obtendrá el valor bajo (byte menos significativo) al igualar 'Vigital' a una variable de 8 bit 'Vlow'.
	
	\item Para todos los motores, se obtendrá el valor alto (byte más significativo) al trasladar el contenido de la variable 'Vdigital' 8 bits a la derecha e igualarlos a una variable de 8 bit 'Vhigh'.
...
Al ejecutar Robocom se envía una rutina por defecto para una configuración previa de la posición de cada motor. Esto se logra presionando la pestaña 'Posiciones por defecto' que se observa en la figura \ref{fig:robocomopen}(sección 1), se producirá la trama de comunicación mostrada en la figura \ref{fig:docklight}. Esto se observó con ayuda del software DockLight v2.2, que es un sniffer/monitor de puerto serial RS232 para Windows.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.65]{img/docklight}
	\caption[Vista de trama transmitida por el software Robocom]{Vista de trama transmitida por el software Robocom al iniciar conexión.}
	\label{fig:docklight}
\end{figure}

Esta trama mostrada en la figura \ref{fig:docklight} indica al brazo que el motor 1 (motor A) está en 0.99 (F! 00 C0), entre otras instrucciones que pueden comprenderse con mayor claridad con ayuda de la tabla \ref{TAB:tramadefault}. Además, se tiene una lista de comandos permitidos en la comunicación del controlador y Robocom, la cual se muestra en la tabla \ref{TAB:comandocliente}.
...
El microcontrolador recibe todas las instrucciones vía puerto serial configurado con 8 bits, 1 bit de parada, 115200 baudios; y utiliza solo los terminales de Rx (U2RX) y Tx(U2TX) para establecer la comunicación. Ante cada instrucción de movimiento, el microcontrolador puede emitir un total de nueve señales PWM. Para los motores A, B, C, les corresponden dos señales, una para movimiento positivo y otra para movimiento negativo, respectivamente. El resto de los motores poseen solo una señal PWM para cada uno. El diagrama de conexiones e identificación de las funciones de cada pin se puede observar en la figura \ref{fig:dspicpinout}.

\section{PWM}
En la figura \ref{fig:dspicpinout} se observan los terminales de salida seleccionados para la señal PWM que controlará a los motores. En el manual anexo al trabajo de Marcano \cite{juan1} indica cuáles son las características básicas que deben poseer los registros de configuración de PWM para el microcontrolador Microchip dsPIC30F3011: 

Motores A, B, C:

Estos motores tienen configurada una señal PWM de 11 bits y frecuencia 29,328kHz. Con esta resolución de señal se puede obtener un ciclo de trabajo máximo configurando el registro de Duty Cycle (PDC) a un valor de 2048. Los terminales de salida de PWM fueron configurados en modo complementario, por lo tanto para que la corriente promedio entregada al motor sea cero se configura el registro PDC=1023. Para que el motor produzca par máximo, el registro de duty cycle debe ser PDC=0 para que gire en un sentido o PDC =2048 para que gire en el otro sentido.
...
\begin{figure}[H]
	...
	\caption{terminales de conexión del módulo controlador original.}
